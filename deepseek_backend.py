...
def build_prompt(property_description: str) -> str: 
     return ( 
         ...
         "【591 專業版】\n" 
         "你必須嚴格按照以下六段式骨架分段輸出，寫成有張力的行銷散文，絕對禁止使用條列式清單（如 單價：xx、總價：xx），漏掉任何一段將視為嚴重的程式碼錯誤：\n" 
         "**[1. 一句抓眼球的標題]**：要包含地標或最大優勢。\n" 
         "**[2. 一句最強差異]**：直接點出這間房跟同區其他物件不一樣的特點。\n" 
         "**[3. 三個買方在意的理由]**：針對物件優勢條列。\n" 
         "**[4. 一段生活場景]**：用描寫式文字寫出居住感。\n" 
         "**[5. 一段完整規格]**：自然地融入坪數、格局、樓層、管理費、車位類型等資訊，缺一不可，不足標示「未提供」。\n" 
         ...
         "4. 【物理抗性轉譯範例鎖死】如果看到公設比高達 37.5% 或戶數稀少，必須轉譯為：【純住 37 戶珍稀高隱私城堡，高公設比換來的是單純鄰里與高得房質感的梯廳，把干擾留給外面，把尊榮留給自己】。其他抗性數字也要比照辦理，轉譯為買方嚮往的畫面。\n" 
         "5. 【逼單金句不准漏】三個渠道的文案結尾，必須 100% 出現『本週僅釋出 3 組帶看名額，歡迎私訊看照片並預約賞屋時段』這句話，少一個字就是嚴重的 Bug！\n" 
